...
class CALC {
	...
	/**
	 * Get variations dependencies
	 *
	 * @param integer $phase_id Phase ID.
	 * @return array
	 */
	public static function get_variations_dependencies( $phase_id ) {
		global $wpdb;

		$sql = $wpdb->prepare(
			"SELECT p.ID as post_id, p.post_title as post_title, pm.meta_value as depends, pm3.meta_value as descvar
			FROM {$wpdb->posts} p
			LEFT JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id AND pm.meta_key = 'pbc_depends'
			LEFT JOIN {$wpdb->postmeta} pm2 ON p.ID = pm2.post_id AND pm2.meta_key = 'pbc_phase'
			LEFT JOIN {$wpdb->postmeta} pm3 ON p.ID = pm3.post_id AND pm3.meta_key = 'pbc_descvar'
			WHERE p.post_type = %s
				AND p.post_status = %s
		...
		// Deserializa el campo depends.
		$variations_dep = $results;
		foreach ( $results as $key => &$row ) {
			$depends = maybe_unserialize( $row['depends'] );

			$variations_dep[ $key ]['post_id'] = (int) $results[ $key ]['post_id'];
			if ( ! is_array( $depends ) ) {
				continue;
			}
			$depends_ids = array();

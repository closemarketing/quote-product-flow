...
class HelperPostTypes {
	...
	public function __construct() {
		...
		add_action( 'add_meta_boxes_qpfw_enquiry', array( $this, 'qpfw_metabox_enquiry' ) );
		add_action( 'save_post_qpfw_enquiry', array( $this, 'save_budget_configuration_lines' ), 10, 2 );
		...
		add_filter( 'manage_edit-qpfw_enquiry_columns', array( $this, 'add_new_budgets_columns' ) );
		add_action( 'manage_qpfw_enquiry_posts_custom_column', array( $this, 'manage_budgets_columns' ), 10, 2 );
	...
	public function qpfw_register_cpt() {
		...
		$labels = array(
			'name'               => __( 'Enquiries', 'quote-product-flow' ),
			'singular_name'      => __( 'Enquiry', 'quote-product-flow' ),
			'add_new'            => __( 'Add Enquiry', 'quote-product-flow' ),
			'add_new_item'       => __( 'Add New Enquiry', 'quote-product-flow' ),
			'edit_item'          => __( 'Edit Enquiry', 'quote-product-flow' ),
			'new_item'           => __( 'New Enquiry', 'quote-product-flow' ),
			'view_item'          => __( 'View Enquiry', 'quote-product-flow' ),
			'search_items'       => __( 'Search for Enquiry', 'quote-product-flow' ),
			'not_found'          => __( 'We didn\'t find any Enquiry', 'quote-product-flow' ),
			'not_found_in_trash' => __( 'We didn\'t find any Enquiry in the trash', 'quote-product-flow' ),
		);
		$args   = array(
			'labels'             => $labels,
			'public'             => false,
		...
		register_post_type( 'qpfw_enquiry', $args );
	}
	 ...
	 * @return void
	 */
	public function qpfw_metabox_enquiry() {
		add_meta_box(
			'enquiry-details',
			__( 'Enquiry Details', 'quote-product-flow' ),
		...
		add_meta_box(
			...
			__( 'Budget Configuration', 'quote-product-flow' ),
	...
	public function render_enquiry_details( $post ) {
		...
		$enquiry_name  = get_post_meta( $post_id, 'qpfw_enquiry_name', true );
		$enquiry_phone = get_post_meta( $post_id, 'qpfw_enquiry_phone', true );
		$enquiry_email = get_post_meta( $post_id, 'qpfw_enquiry_email', true );
		$enquiry_city  = get_post_meta( $post_id, 'qpfw_enquiry_city', true );
		$enquiry_state = get_post_meta( $post_id, 'qpfw_enquiry_state', true );
		$comments      = get_post_meta( $post_id, 'qpfw_enquiry_comments', true );
		$parent_phase  = (int) get_post_meta( $post_id, 'qpfw_parent_phase', true );
		?>
		<div><label><strong><?php esc_html_e( 'Enquiry ID:', 'quote-product-flow' ); ?></strong> <?php echo esc_html( $post_id ); ?></label></div>
		<?php if ( ! empty( $enquiry_name ) ) { ?>
			<div><label><strong><?php esc_html_e( 'Name:', 'quote-product-flow' ); ?></strong> <?php echo esc_html( $enquiry_name ); ?></label></div>
		<?php } ?>
		<?php if ( ! empty( $enquiry_phone ) ) { ?>
			<div><label><strong><?php esc_html_e( 'Phone:', 'quote-product-flow' ); ?></strong> <?php echo esc_html( $enquiry_phone ); ?></label></div>
		<?php } ?>
		<?php if ( ! empty( $enquiry_email ) ) { ?>
			<div><label><strong><?php esc_html_e( 'Email:', 'quote-product-flow' ); ?></strong> <?php echo esc_html( $enquiry_email ); ?></label></div>
		<?php } ?>
		<?php if ( ! empty( $enquiry_city ) ) { ?>
			<div><label><strong><?php esc_html_e( 'City:', 'quote-product-flow' ); ?></strong> <?php echo esc_html( $enquiry_city ); ?></label></div>
		<?php } ?>
		<?php if ( ! empty( $enquiry_state ) ) { ?>
			<div><label><strong><?php esc_html_e( 'State:', 'quote-product-flow' ); ?></strong> <?php echo esc_html( $enquiry_state ); ?></label></div>
		<?php } ?>
		<?php if ( ! empty( $enquiry_state ) ) { ?>
			<div><label><strong><?php esc_html_e( 'Comments:', 'quote-product-flow' ); ?></strong> <?php echo esc_html( $comments ); ?></label></div>
		<?php } ?>
		<?php
		if ( $parent_phase ) {
			$parent_phase_post = get_post( $parent_phase );
			echo '<div><label><strong>' . esc_html__( 'Product:', 'quote-product-flow' ) . '</strong> ' . esc_html( $parent_phase_post->post_title ) . '</label></div>';
		}
	}
	/**
	...
	public function render_budget_configuration( $post ) {
		...
		$rows = array();
		for ( $i = 0; $i < 50; $i++ ) {
			$phase_var = get_post_meta( $post_id, 'qpfw_phase_var_' . $i, true );
			if ( $phase_var ) {
				$rows[] = array(
					'desc'       => $phase_var,
					'price'      => get_post_meta( $post_id, 'qpfw_price_' . $i, true ),
					'type'       => get_post_meta( $post_id, 'qpfw_type_' . $i, true ),
					'phase_name' => get_post_meta( $post_id, 'qpfw_phase_name_' . $i, true ),
				);
			}
		}
		if ( empty( $rows ) ) {
		...
		$phase_options = CALC::get_phases_options();
		wp_nonce_field( 'qpfw_budget_config_save', 'qpfw_budget_config_nonce' );
		?>
		<p class="description" style="margin-bottom:12px;">
			<?php esc_html_e( 'Lines mirror the configurator: standard options (price), quantity fields, free-text questions, and phases with multiple selections (one combined line). You can insert from the catalog or type manually.', 'quote-product-flow' ); ?>
		</p>
		<?php if ( ! empty( $phase_options ) ) : ?>
		<div class="qpfw-catalog-picker" style="margin:12px 0;padding:12px;background:#f6f7f7;border:1px solid #c3c4c7;border-radius:4px;">
			<strong><?php esc_html_e( 'Insert from catalog', 'quote-product-flow' ); ?></strong>
			<p style="margin:8px 0 6px;">
				<label for="qpfw-catalog-phase" style="display:inline-block;min-width:90px;"><?php esc_html_e( 'Phase', 'quote-product-flow' ); ?></label>
				<select id="qpfw-catalog-phase" style="min-width:280px;">
					<option value=""><?php esc_html_e( 'Select phase…', 'quote-product-flow' ); ?></option>
					<?php foreach ( $phase_options as $pid => $plabel ) : ?>
						<option value="<?php echo esc_attr( (string) $pid ); ?>"><?php echo esc_html( $plabel ); ?></option>
					<?php endforeach; ?>
				</select>
				<span id="qpfw-catalog-phase-hint" style="margin-left:8px;color:#50575e;"></span>
			</p>
			<div id="qpfw-catalog-multi-mode" style="display:none;margin-top:10px;padding-top:10px;border-top:1px solid #c3c4c7;">
				<strong><?php esc_html_e( 'Multiple selection (combined in one line)', 'quote-product-flow' ); ?></strong>
				<p class="description"><?php esc_html_e( 'Select several price options and insert one line with all names and the sum of prices (same as when the customer ticks several boxes).', 'quote-product-flow' ); ?></p>
				<div id="qpfw-catalog-multi-list" style="max-height:220px;overflow:auto;margin:8px 0;padding:8px;background:#fff;border:1px solid #c3c4c7;"></div>
				<button type="button" class="button button-primary" id="qpfw-insert-multi-row"><?php esc_html_e( 'Insert combined selection', 'quote-product-flow' ); ?></button>
			</div>
			<div id="qpfw-catalog-single-mode" style="margin-top:12px;">
				<strong><?php esc_html_e( 'Single option or question', 'quote-product-flow' ); ?></strong>
				<p style="margin:6px 0;">
					<label for="qpfw-catalog-variation" style="display:inline-block;min-width:90px;"><?php esc_html_e( 'Variation', 'quote-product-flow' ); ?></label>
					<select id="qpfw-catalog-variation" style="min-width:280px;">
						<option value=""><?php esc_html_e( 'Select variation…', 'quote-product-flow' ); ?></option>
					</select>
					<span id="qpfw-catalog-price-hint" style="margin-left:8px;color:#50575e;"></span>
				</p>
				<p id="qpfw-catalog-answer-wrap" style="display:none;margin:6px 0;">
					<label for="qpfw-catalog-answer" style="display:inline-block;min-width:90px;"><?php esc_html_e( 'Answer', 'quote-product-flow' ); ?></label>
					<input type="text" id="qpfw-catalog-answer" class="regular-text" placeholder="<?php esc_attr_e( 'Customer answer (same format as on the website)', 'quote-product-flow' ); ?>" />
				</p>
				<p id="qpfw-catalog-qty-wrap" style="display:none;margin:6px 0;">
					<label for="qpfw-catalog-qty" style="display:inline-block;min-width:90px;"><?php esc_html_e( 'Quantity', 'quote-product-flow' ); ?></label>
					<input type="number" id="qpfw-catalog-qty" min="1" value="1" style="width:80px;" />
					<span class="description"><?php esc_html_e( 'Total price = quantity × unit price.', 'quote-product-flow' ); ?></span>
				</p>
				<button type="button" class="button" id="qpfw-insert-catalog-row"><?php esc_html_e( 'Insert row', 'quote-product-flow' ); ?></button>
			</div>
		</div>
		<?php else : ?>
		<p class="notice notice-warning inline" style="padding:8px 12px;">
			<?php esc_html_e( 'No phases found. Create phases and variations under Quote Product Flow so you can build budgets from the catalog.', 'quote-product-flow' ); ?>
		</p>
		<?php endif; ?>
		<table class="widefat striped" style="margin-top:12px;">
			<thead>
				<tr>
					<th style="width:42%;"><?php esc_html_e( 'Phase / option (description)', 'quote-product-flow' ); ?></th>
					<th style="width:18%;"><?php esc_html_e( 'Price', 'quote-product-flow' ); ?></th>
					<th style="width:22%;"><?php esc_html_e( 'Line type', 'quote-product-flow' ); ?></th>
					<th style="width:18%;"><?php esc_html_e( 'Actions', 'quote-product-flow' ); ?></th>
				</tr>
			</thead>
			<tbody id="qpfw-budget-rows">
				<?php foreach ( $rows as $row ) : ?>
				<tr>
					<td>
						<input type="text" class="widefat qpfw-row-desc" name="qpfw_line_desc[]" value="<?php echo esc_attr( $row['desc'] ); ?>" />
						<input type="hidden" class="qpfw-row-phase-name" name="qpfw_line_phase_name[]" value="<?php echo esc_attr( $row['phase_name'] ); ?>" />
					</td>
					<td>
						<input type="text" class="widefat qpfw-row-price" name="qpfw_line_price[]" value="<?php echo esc_attr( $row['price'] ); ?>" placeholder="0,00 / -" />
					</td>
					<td>
						<select class="widefat qpfw-row-type" name="qpfw_line_type[]">
							<?php
							$lt = isset( $row['type'] ) ? (string) $row['type'] : '';
							$line_types = array(
								''          => __( 'Default (price)', 'quote-product-flow' ),
								'price'     => __( 'Fixed price option', 'quote-product-flow' ),
								'qty'       => __( 'Quantity × unit', 'quote-product-flow' ),
								'question'  => __( 'Question / answer', 'quote-product-flow' ),
								'multiple'  => __( 'Multiple options (combined)', 'quote-product-flow' ),
							);
					...
					<td>
						<button type="button" class="button-link-delete qpfw-remove-budget-row"><?php esc_html_e( 'Remove', 'quote-product-flow' ); ?></button>
					</td>
				</tr>
				<?php endforeach; ?>
			</tbody>
		...
		<p style="margin-top:10px;">
			<button type="button" class="button qpfw-add-budget-row"><?php esc_html_e( 'Add empty row', 'quote-product-flow' ); ?></button>
		</p>
		<script type="text/template" id="qpfw-budget-row-template">
			<tr>
				<td>
					<input type="text" class="widefat qpfw-row-desc" name="qpfw_line_desc[]" value="" />
					<input type="hidden" class="qpfw-row-phase-name" name="qpfw_line_phase_name[]" value="" />
				</td>
				<td><input type="text" class="widefat qpfw-row-price" name="qpfw_line_price[]" value="" placeholder="0,00 / -" /></td>
				<td>
					<select class="widefat qpfw-row-type" name="qpfw_line_type[]">
						<option value=""><?php echo esc_html__( 'Default (price)', 'quote-product-flow' ); ?></option>
						<option value="price"><?php echo esc_html__( 'Fixed price option', 'quote-product-flow' ); ?></option>
						<option value="qty"><?php echo esc_html__( 'Quantity × unit', 'quote-product-flow' ); ?></option>
						<option value="question"><?php echo esc_html__( 'Question / answer', 'quote-product-flow' ); ?></option>
						<option value="multiple"><?php echo esc_html__( 'Multiple options (combined)', 'quote-product-flow' ); ?></option>
					</select>
				</td>
				<td><button type="button" class="button-link-delete qpfw-remove-budget-row"><?php echo esc_html__( 'Remove', 'quote-product-flow' ); ?></button></td>
			</tr>
		</script>
		<?php
	}
	...
	public function enqueue_enquiry_budget_admin_assets( $hook_suffix ) {
		...
		if ( ! $screen || 'qpfw_enquiry' !== $screen->post_type ) {
			return;
		}
		wp_enqueue_script(
			'qpfw-admin-enquiry-budget',
			QPFW_PLUGIN_URL . 'includes/assets/qpfw-admin-enquiry-budget.js',
			array( 'jquery' ),
			QPFW_VERSION,
			true
		);
		wp_localize_script(
			...
			array(
				...
				'i18n'            => array(
					'selectVariation'   => __( 'Select variation…', 'quote-product-flow' ),
					'suggestedPrice'    => __( 'Suggested price:', 'quote-product-flow' ),
					'phaseMulti'        => __( 'Multiple selection phase', 'quote-product-flow' ),
					'needAnswer'        => __( 'Enter the answer for this question.', 'quote-product-flow' ),
					'pickMulti'         => __( 'Select at least one option.', 'quote-product-flow' ),
					'noOptionsMulti'    => __( 'No price options in this phase (only questions?). Add questions using single mode on another step or a manual row.', 'quote-product-flow' ),
					'perUnit'           => __( 'per unit', 'quote-product-flow' ),
				),
			)
		);
	}
	...
	private function get_phase_variations_admin_data() {
		...
		foreach ( $variationscpt as $var_item ) {
			$phase_id = (int) get_post_meta( $var_item->ID, 'qpfw_phase', true );
			if ( ! $phase_id ) {
				continue;
			}
			$phase_post = get_post( $phase_id );
			...
			$pricegroup   = get_post_meta( $var_item->ID, 'qpfw_pricegroup', true );
			$unit         = 0.0;
			$first_mea    = '';
			if ( ! empty( $pricegroup ) && is_array( $pricegroup ) ) {
				foreach ( $pricegroup as $row ) {
					if ( isset( $row['qpfw_pricem'] ) && '' !== $row['qpfw_pricem'] && is_numeric( $row['qpfw_pricem'] ) ) {
						$unit      = (float) $row['qpfw_pricem'];
						$first_mea = isset( $row['qpfw_meaprice'] ) ? (string) $row['qpfw_meaprice'] : '';
						break;
					}
				}
			}
			...
			$is_question = (bool) get_post_meta( $var_item->ID, 'qpfw_is_question', true );
			$field_type  = get_post_meta( $var_item->ID, 'qpfw_field_type', true );
			if ( $is_question ) {
				$line_label .= ' — ' . __( 'Question', 'quote-product-flow' );
			} elseif ( 'qty' === $field_type ) {
				$line_label .= ' — ' . __( 'Quantity field', 'quote-product-flow' );
			}
	...
	private function get_phases_admin_flags() {
		...
		foreach ( array_keys( CALC::get_phases_options() ) as $pid ) {
			...
			$flags[ (string) $pid ] = array(
				'allowMultiple' => (bool) get_post_meta( $pid, 'qpfw_allow_multiple_selections', true ),
	...
	public function save_budget_configuration_lines( $post_id, $post ) {
		if ( ! isset( $_POST['qpfw_budget_config_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['qpfw_budget_config_nonce'] ) ), 'qpfw_budget_config_save' ) ) {
			return;
		}
		// Block editor / REST saves do not send metabox fields; never wipe stored lines.
		if ( ! isset( $_POST['qpfw_line_desc'] ) || ! is_array( $_POST['qpfw_line_desc'] ) ) {
			return;
		}
		if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
			return;
		}
		if ( ! $post || 'qpfw_enquiry' !== $post->post_type ) {
			return;
		}
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return;
		...
		for ( $c = 0; $c < 120; $c++ ) {
			delete_post_meta( $post_id, 'qpfw_phase_var_' . $c );
			delete_post_meta( $post_id, 'qpfw_price_' . $c );
			delete_post_meta( $post_id, 'qpfw_phase_name_' . $c );
			delete_post_meta( $post_id, 'qpfw_type_' . $c );
		}

		$descs        = isset( $_POST['qpfw_line_desc'] ) ? wp_unslash( $_POST['qpfw_line_desc'] ) : array();
		$prices       = isset( $_POST['qpfw_line_price'] ) ? wp_unslash( $_POST['qpfw_line_price'] ) : array();
		$types        = isset( $_POST['qpfw_line_type'] ) ? wp_unslash( $_POST['qpfw_line_type'] ) : array();
		$phase_names  = isset( $_POST['qpfw_line_phase_name'] ) ? wp_unslash( $_POST['qpfw_line_phase_name'] ) : array();
		if ( ! is_array( $descs ) ) {
			$descs = array();
		}
		if ( ! is_array( $prices ) ) {
		...
		for ( $r = 0; $r < $max; $r++ ) {
			...
			update_post_meta( $post_id, 'qpfw_phase_var_' . $index, $desc );
			update_post_meta( $post_id, 'qpfw_price_' . $index, $price );
			update_post_meta( $post_id, 'qpfw_type_' . $index, $line_type );
			update_post_meta( $post_id, 'qpfw_phase_name_' . $index, $phase_n );
			++$index;
		}
		update_post_meta( $post_id, 'qpfw_total_var', $index );
	}
	...
	public function add_new_budgets_columns( $phases_columns ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
		...
		unset( $phases_columns );
		$new_columns['cb']              = '<input type = "checkbox" />';
		$new_columns['enquiry_name']    = __( 'Budget', 'quote-product-flow' );
		$new_columns['enquiry_details'] = __( 'Details', 'quote-product-flow' );
		$new_columns['enquiry_conf']    = __( 'Configuration', 'quote-product-flow' );
		$new_columns['enquiry_date']    = __( 'Date', 'quote-product-flow' );
	...
	public function manage_budgets_columns( $column_name, $id ) {
		switch ( $column_name ) {
			case 'enquiry_name':
				...
				$name = get_post_meta( $id, 'qpfw_enquiry_name', true );
				if ( empty( $name ) ) {
			...
			case 'enquiry_details':
				...
				break;
			case 'enquiry_conf':
				echo number_format( CALC::get_total_from_enquiry( $id ), 2, ',', '.' ) . ' € ' . esc_html__( 'VAT not included', 'quote-product-flow' );
				break;
			case 'enquiry_date':
				echo get_the_date( 'd-m-Y H:i', $id );
				echo '<br/><button class="button generate-qpfw-pdf" data-post-id="' . (int) $id . '">';
				esc_html_e( 'Generate PDF', 'quote-product-flow' );
				echo '</button><span id="qpfw-pdf-' . (int) $id . '" class="spinner"></span>';
				break;
			default:
				break;
		} // end switch
	...
	public function add_duplicate_row_action( $actions, $post ) {
		$qpfw_post_types = array( 'qpfw_phases', 'variation', 'enquiry' );
